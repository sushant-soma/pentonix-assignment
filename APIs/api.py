...
# Create in-memory databases as dictionaries for departments and employees.
# Each department has a unique department number (DNO) associated with it.
departments = {
    10: 'Admin',
    20: 'Accounts',
    30: 'Sales',
...
# Each employee has a unique employee number (ENO) and is associated with a department using DNO.
# The 'SALARY' field represents the salary of the employee.
employees = {
...
# API endpoint for retrieving employee details by ENO.
@app.route('/api', methods=['GET'])
def get_employee_by_eno_or_dname():
    try:
        # Extract the 'ENO' and 'DNAME' query parameters from the request URL.
...
# Start the Flask application on port 9000.
